...
local function create_floating_file(location, opts)
  ...
  local contents = vim.api.nvim_buf_get_lines(
    ...
    range.start.line,
    math.min(
    -- range["end"].line + 1 + (opts.context or plugins.lsp.peek.max_height),
    -- range.start.line + (opts.max_height or plugins.lsp.peek.max_height)
      3, 3
    ),
    false
  )
  ...
  -- if_nil(width, plugins.lsp.peek.max_width),
  -- if_nil(height, plugins.lsp.peek.max_height),
    if_nil(width, 3),
    if_nil(height, 3),
    opts
  )
...
local function preview_location_callback(result)
  ...
  local opts = {
    border = "rounded",
    -- context = plugins.lsp.peek.context,
    context = 'sdf',
  }
...
function M.Peek(what)
  ...
  if vim.tbl_contains(vim.api.nvim_list_wins(), M.floating_win) then
    ...
    vim.api.nvim_buf_set_keymap(
      ...
      "n",
      "<CR>",
      ":lua require('plugins.lsp.peek').open_file()<CR>",
